...
Existing workstream cards now open the Planning workstream drawer rather than the New Merch review modal. They no longer show `Create Ecomm Workstream`/`Create Packaging Workstream` or Previous/Next Merchandise controls; those actions remain limited to New Merch intake. The workstream drawer retains the Product-data save action and available queue actions.
...
The Dashboard now includes a compact `Newly Received Merch` section sourced from active Shipment merchandise entries. It shows newly received merchandise as item-level cards with photo, client, name, UPC / ID, match state, and a `New` marker, and links users into Planning for review. This is a dashboard visibility slice only; it does not create a new workflow status or duplicate the Planning board.
...
Topco defaults are Packaging: Product Name, UPC/Product ID, Job Number, Brand Prefix, and File Name Description, with `{jobNumber}_{brandPrefix}_{fileNameDescription}`; and Ecomm: Product Name, UPC/Product ID, CVID, and Job Number, with `{cvid}_{view}` and the standard front, back, left, right, top, bottom, front elevated, left elevated, and right elevated views.

The backend returns a derived `photoProduction` summary on workstream cards. It reports missing Product values and filename tokens/views. Creative Force owns final naming execution; Marks Photo verifies that the configured values exist for handoff. `Brand Prefix` is the packaging token, not generic Brand. Quantity received and physical production statuses remain Received Merch or downstream production data, and Activation remains separate. File Name Description is currently resolved from Product Reference Data when present.
...
The first Creative Force return-status slice is now present in the backend. `POST /api/integrations/creative-force/webhook` accepts signed Creative Force Work Unit events using `X-CF-Signature` and `CREATIVE_FORCE_WEBHOOK_SECRET`, matches events by `WorkUnitId`, normalizes the reported status, and stores the result in the app-owned Workstream Card field `Creative Force Sync`. It does not change Planning status, Product data, or physical Merchandise status. The Airtable field is expected but has not been provisioned in this slice; outbound handoff creation and reconciliation polling remain future work.
...
Planning now has a deliverable filter for Ecomm, Packaging, and Thr3d, with distinct Ecomm and Packaging badge treatments. A Kanban/List toggle provides an alternate grouped list view using the same queue columns and card detail behavior.
...
The Planning review modal now uses the numbered Product data for photo step as the single heading. The editor no longer repeats that heading or adds a redundant divider; it retains the supporting instruction, editable fields, and save action.
The Product Data editor now hides its explanatory copy and places Save product data below the required fields so the mandatory inputs remain the visual focus.
Save product data now remains visible at the bottom of the scrollable editor and explicitly calls the backend `/products/:id` update route. The backend persists the mapped values to the Products table, with File Name Description retained in Product Reference Data.
...
The local backend on port 5057 was restarted on 2026-08-12 so the active browser session is serving the current Products PATCH route. Route-level tests cover Product Name/ID-adjacent edits, Job Number, Brand Prefix, and File Name Description persistence.
...
The live Airtable Products table still contains an obsolete `Merch Status` field, but active Product shaping does not use it. Merchandise `Merch Status` remains the only physical status source.

Existing workstream cards no longer show `In Creative Force` as a dead-end action. Product data remains editable through Save product data, Add activation opens the shared Activation editor with the current merchandise pre-linked, and the footer primary action appears only for an actual workstream change or valid move to Ready for Photo.
The shared card footer now labels a permitted workstream change as `Change to Ecomm` or `Change to Packaging`, labels a valid progression as `Move to Ready for Photo`, and places the add/change/remove rules beneath the action row. When both child workstreams exist, the card explains that the other workstream is already tied to the merchandise and only removal is available.

Creative Force direct Airtable ingestion is separated from the Product workspace. The physical Airtable table `Creative Force Product Feed` was provisioned on 2026-08-12 as `tblxEmSy1xZLHtEWW`. It contains fixed handoff fields plus the union of Product fields required by client Photo Production Requirements. The current live dynamic fields include `Product Name`, `UPC / Product ID`, `CVID`, `Job Number`, `Brand Prefix`, `File Name Description`, and `Valid Artwork Path`. The schema utility adds a new scalar column when a client configuration introduces another supported required Product field. It is not a status, blocker, or reporting table.

When an Ecomm or Packaging Workstream Card moves to `Ready for Photo`, the backend automatically upserts one flat row into the `Creative Force Product Feed` table. There is no manual sync action. Incomplete cards are omitted; the feed contains only Creative Force input fields and is not a status, blocker, or reporting table. The physical table is provisioned; Creative Force Connector configuration remains future work.
