...
When a New Merch Planning modal has a linked Product with `Request Type` and the Merchandise has no committed `Deliverables`, the modal preselects draft Deliverables from the Product request: `Ecomm only` -> `Ecomm`, `Pack only` -> `Packaging`, `Thr3d only` -> `Thr3d`, `Pack & Thr3d` -> `Packaging` + `Thr3d`, and `Ecomm & Pack` -> `Ecomm` + `Packaging`. The modal displays the Product-derived suggestion and still follows Draft -> Commit: the selection remains draft state until `Confirm Merch`, and the PM can change it before committing. The Planning Release view's `Newly Received Merch` cards stay identity-focused and do not show suggested Packaging/Ecomm/Thr3d badges or deliverables-missing text before PM review. They show the item title plus UPC / ID and matched/unmatched state; deliverable and missing-info messaging belongs after work has been declared in the later Planning sections. The `New` age pill appears only in `Newly Received Merch`, not in later Planning sections whose merchandise has already been validated. The Release view has a `Group by shipment` display option that groups only `Newly Received Merch` and `Needs More Information`; `Ready to Release` remains ungrouped. Group headers show only the right-aligned shipment received date/time, without a duplicate left-side timestamp or repeated client prefix. When a grouped Planning item has a linked Shipment id, that received date/time links to `/shipments?shipmentId=...`; the Shipments page opens that Shipment on load.
...
The Awaiting Info/Activation workstream-card drawer now renders the same editable Product Data fields as the merchandise review card, scoped to the card's selected deliverable and linked Expected Product. Saving from either surface updates the shared Product record.
...
Existing workstream cards open the shared Planning detail modal rather than presenting New Merch intake controls. They no longer show create-workstream or Previous/Next Merchandise controls. The detail surface keeps Product/photo data editable through `Save Details`; release happens from the Ready-to-Release batch surface.
The workstream drawer now has a dedicated compact card surface: deliverable/client/product context, editable required photo Product fields, merchandise facts, and only the Ready for Photo transition when available. It no longer falls through to the generic Waiting information workspace.
When those checks are complete, `Ready for Photo` opens the shared Planning photo-release utility with the current item already selected. The PM can release that item alone or add other eligible ready items, review the shared request details and email preview, save a draft without releasing anything, or commit the selected group with `Ready for Photo`. The visible flow no longer presents Activation as an individual-card requirement.
...
Admin > Developer Tools now exposes `Reset Test Data` as the development-only clean-slate action. It deletes workflow/testing records from Comments, Workstream Cards, THR3D Shipping Items, Activations, Issues, History, Imports, Merchandise, Shipments, and Jobs, and it attempts to delete only the R2 uploaded photo objects referenced by the deleted Shipment and Merchandise records. Products, Clients, Users, Locations, Airtable schema, field options, and client configuration are intentionally preserved.
...
Legacy intake labels are API compatibility aliases only. The active persisted planning field is `Planning Status`, with these canonical values:
...
The canonical first Planning queue remains `New` internally, and the board now displays it as `New Merch` so brand-new received merchandise has a clean inbox before PM review. Cards in that column may need a physical acceptance check, product matching, deliverables, activation, or another first PM decision. The column cards are intentionally sparse: item photo with subtle remaining-image count and age overlays, quiet client eyebrow, one-line product name, `UPC / ID`, quantity using the same compact label/value treatment as UPC / ID, and a single match-state marker. The match marker says `Matched` with a green check when linked to a Product and `Unmatched` when not linked. The visible action is not duplicated as a board-card badge; opening the card enters the review modal, where the first step is `Merch Check`, the primary footer action is `Confirm Merch`, and the footer preview explains whether the item will move to `Needs More Information`, move to `Ready to Release`, or remain blocked by an Issue.
...
The Ready for Photo activation/release form now shows item-level Artwork Path and Upload Location for both Ecomm and Packaging releases, restores Structure for both, and carries client-configured Product requirements into each item row. Ecomm item rows include CVID; Packaging rows include Product Name/Description, Brand Prefix, WKFT Job Number, and File Name Description when configured, with values prefilled from the linked Product. The email preview lists artwork and upload locations in separate per-item sections before SKU Details. Existing shared path values are copied into item rows when older releases are reopened.

The release header now reads `Ready for Photo:` followed by the selected Ecomm/Packaging deliverable badge. The scoped card flow hides the redundant deliverable selector, while grouped releases retain it. The shared project field is labeled `Project name`.

The Ready for Photo release UI no longer presents saved drafts or a Save Draft action. The form commits only through `Ready for Photo`; legacy backend Draft activation records remain readable for compatibility but are not part of the new creation flow.
...
Selecting `No clear match` in New Merch now clears the linked Expected Product, sets the Merchandise to `Waiting on Information`, and removes the matched state from the card. The PM can search and choose a Product again later.

Planning cards now use an age label in the upper-right media corner instead of the former status-dot indicators. New Merch and Needs Product / Work cards show the first-step checks for Product match and deliverables. Awaiting Info/Activation cards show only the next-step checks: Photo Data and Activation. Ready for Photo remains the destination after those selected-deliverable requirements are satisfied.
...
The Planning Release view is now the default Planning view. It does not add workflow states, schema, routes, or new persistence. It regroups the same filtered active Planning cards into `Newly Received Merch`, `Needs More Information`, and `Ready to Release` sections. `Newly Received Merch` is the first-review inbox: merchandise there must be viewed and acknowledged before it can leave that section, even if captured data is already complete. Once reviewed, cards with outstanding validation go to `Needs More Information`. Ready-to-release eligibility uses the same readiness signal as the card modal's detail action: Product/manual identity present, photo deliverable present, and configured Product Data complete. Complete accepted cards rise into this section because they only need Activation/release details before being committed to the `Ready for Photo` handoff. Existing Ecomm and Packaging child workstream cards remain supported for compatibility, but the Release view should visually keep one merchandise card through `Newly Received Merch` and `Needs More Information` until release/handoff. Cards already committed to `Ready for Photo` leave the default active Planning view; downstream production status belongs on the Production-facing surfaces. Cards in `Ready to Release` can be selected in batches by clicking the card itself; selected cards use a strong visual selected treatment rather than checkboxes. `Create Release` opens the existing activation/release modal with one item row per selected Merchandise record. A small pencil/Edit action on ready cards opens the shared Planning modal.
...
Awaiting Info/Activation cards now show Photo Data checks for both Received Merch cards and child workstream cards. The media `+1` photo-count marker is intentionally limited to New Merch cards; activation is handled by the Planning-level photo release action after card data is complete.
...
Planning card detail surfaces are now unified: New Merch, Received Merch, and Ecomm/Packaging workstream cards open the same review modal with the same merchandise photos, Product match, Deliverables, Product data, comments, and history anatomy. Queue position controls the available footer action only. New Merch retains Previous/Next Merchandise navigation and the review/assignment action; existing workstream cards do not show merchandise navigation or a create action and can move to Ready for Photo when their configured Product validation is complete.
...
When an Ecomm or Packaging Workstream Card moves to `Ready for Photo`, the backend automatically upserts one flat row into the `Creative Force Product Feed` table. Client Photo Production Requirements are the single gate for entering `Ready for Photo`; the feed writer does not apply a second hard-coded handoff validation gate after that status is committed. There is no manual sync action. The feed contains only Creative Force input fields and is not a status, blocker, or reporting table. The physical table is provisioned; Creative Force Connector configuration remains future work.

The Planning-level Edit Photo Group `Move to Photo` path also reconciles every linked Ecomm or Packaging card that is already `Ready for Photo`. Releasing or re-saving a group therefore cannot bypass the automatic Creative Force Product Feed upsert simply because a child card's status did not change during that request.

When a Ready for Photo release is opened from a specific Ecomm or Packaging card, the release is scoped to that child workstream. The parent Received Merch record keeps its aggregate deliverables, while any unselected sibling remains in its current queue; the parent itself is not marked Ready for Photo until no photo sibling remains unreleased.

Planning queue placement is now exposed as one normalized API field, `planningStatus`, with the values `new`, `needs-product-work`, `awaiting-info`, and `ready-for-photo`. The board no longer reads or writes browser-local queue overrides. Legacy Merchandise intake fields and Workstream Card status remain compatibility storage behind the API; the frontend uses the normalized field for the four canonical columns. Explicit queue writes accept `planningStatus` and translate to the existing Airtable fields, while the Ready for Photo commit remains the controlled handoff that populates the Creative Force Product Feed.

New shipment merchandise with `Planning Status` set to `New` remains in the New Merch queue even when raw product or deliverable values are already present from intake. Those values do not count as PM review. Explicit `Awaiting Info`, `Ready for Photo`, and `Workflows Created` states continue to route through their corresponding planning queues. Older `Intake Status` labels are accepted only as inbound compatibility aliases and are translated before persistence.

Topco Ecomm workstream cards now carry the same linked Activation relationship used by Received Merch cards. Their shared review modal treats Activation as the final Ready for Photo gate, so it does not offer a misleading move action while the visible Step 4 still says Add activation.
The Planning card flow no longer presents Activation as an individual-card step. Workstream cards show and edit only match, deliverables, and configured Product/photo data; when those checks are complete, the card exposes one `Ready for Photo` action. That action opens the Planning-level photo release editor with the current item preselected. The PM may send that item alone or add other eligible ready items to the same release, provide the release details, and commit the group to Ready for Photo. The existing one-time Activation record and `move-to-photo` endpoint remain the persistence mechanism underneath, but the visible workflow is a photo release rather than an activation step.

Admin now has a visible Creative Force section with a live feed preview. It shows that the Airtable feed table is provisioned and previews rows already created for Ready for Photo cards. Refresh is read-only; there is no manual sync control. It does not present blocked items as feed rows; those remain in Products and Planning.

The shared card footer now exposes `Add Deliverable` when exactly one photo deliverable exists and the sibling is missing. It creates only the missing Ecomm or Packaging child and closes the modal; it does not reroute, replace, or downgrade the existing card, including when that card is already Ready for Photo. Once both photo workstreams exist, the add action is unavailable.
...
When a photo group is moved to Ready for Photo, the backend updates both the linked Merchandise intake record and its Ecomm/Packaging Workstream Card statuses. THR3D remains a shipping record and is excluded from photo-card release.
...
The Ready for Photo release modal now derives its visible item fields, artwork/upload fields, and email-preview SKU columns from the selected client's configured photo-production requirements and selected deliverables. Fields that are neither configured nor populated are omitted from the release package. Product values may be inherited from matched merchandise and now render as compact read-only validated details inside each item row instead of editable inputs; Planning remains the place to correct validated item/Product data. Artwork path and upload location are release-package fields and remain editable in the modal. Structure remains available as optional release context. The modal preview uses the selected client and deliverable label rather than a hard-coded Topco Ecomm layout. New photo releases are scoped to one photo deliverable at launch, so the modal no longer shows an internal Deliverables selector. Ready-to-release batching prevents mixed Ecomm and Packaging selection; PMs release those deliverables separately.
...
The Ready for Photo release editor marks the same mandatory project and item fields used by its readiness validation with red asterisks. Completed mandatory labels, including configured client fields and path/structure requirements, turn gray as their values are entered.
