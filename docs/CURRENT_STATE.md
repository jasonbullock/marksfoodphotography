...
Moving an Ecomm or Packaging workstream card on the Planning board updates that child card's own `Workstream Cards.Status` through `PATCH /api/workstream-cards/:id`. The active child statuses are `New`, `Needs More Information`, and `Awaiting Photo Release`; board placement follows those same three Planning buckets. Moving a child workstream card does not rewrite the parent Received Merch physical status. The backend writes status updates with Airtable `typecast` enabled. Airtable's public metadata update endpoint returned 422 when asked to prune retired dropdown options, so the visible Airtable field configuration may still need manual adjustment.
