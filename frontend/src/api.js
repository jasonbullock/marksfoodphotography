...
api.previewMerchandiseTag = async entryId => api.printMerchandiseTag(entryId, { preview: true });
...
api.validateVerificationEntry = async (entryId, status = 'Validated') => backend('POST', `/verification/entries/${entryId}/validate`, { status });
...
api.validateMerchandiseReviewEntry = async (entryId, status = 'Validated') => backend('POST', `/merchandise/review/${entryId}/validate`, { status });
api.updateMerchandiseStatus = async (entryId, status) => backend('POST', `/merchandise/review/${entryId}/validate`, { status });
...
api.listRecentComments = async (limit = 8) => backend('GET', `/comments/recent?limit=${encodeURIComponent(limit)}`);
