...
from routes import evaluate_photo_readiness, _validate_identifier_value  # noqa: E402
...
class ReadinessTests(unittest.TestCase):
    ...
    def test_leading_zero_is_preserved_and_valid(self):
        ...
        result = evaluate_photo_readiness(item, self.client(), [])
    ...
    def test_required_field_evaluation_uses_client_label(self):
        result = evaluate_photo_readiness(self.item(productId="", identifier=""), self.client(identifierLabel="UPC"), [])
    ...
    def test_artwork_requirement_states(self):
        required = evaluate_photo_readiness(self.item(artworkReceived=False), self.client(artworkRequirement="Required"), [])
        self.assertEqual(required["state"], "missing_artwork")

        optional = evaluate_photo_readiness(self.item(artworkReceived=False), self.client(artworkRequirement="Optional"), [])
        self.assertEqual(optional["state"], "ready_for_photo")
        self.assertTrue(optional["warnings"])

        not_needed = evaluate_photo_readiness(self.item(artworkReceived=False), self.client(artworkRequirement="Not Needed"), [])
    ...
    def test_missing_unidentified_and_valid_merchandise(self):
        missing = evaluate_photo_readiness(self.item(received=False, receiptIds=[]), self.client(), [])
        self.assertEqual(missing["state"], "waiting_for_merchandise")

        unidentified = evaluate_photo_readiness(self.item(productId="", identifier="", received=True), self.client(), [])
        self.assertEqual(unidentified["state"], "missing_data")

        logged_only = evaluate_photo_readiness(self.item(received=False, receiptIds=["recReceipt"]), self.client(), [])
        self.assertEqual(logged_only["state"], "waiting_for_merchandise")

        valid = evaluate_photo_readiness(self.item(received=True, receiptIds=["recReceipt"]), self.client(), [])
    ...
    def test_blocking_and_resolved_merchandise_issues(self):
        ...
        blocked = evaluate_photo_readiness(self.item(), self.client(), [issue])
        self.assertEqual(blocked["state"], "merchandise_issue")

        resolved = evaluate_photo_readiness(self.item(), self.client(), [{**issue, "status": "Resolved"}])
        self.assertEqual(resolved["state"], "ready_for_photo")

    def test_completed_items_do_not_move_backward(self):
        result = evaluate_photo_readiness(
