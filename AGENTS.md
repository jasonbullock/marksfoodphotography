...
Workspace polish passes must preserve the canonical public milestones (`Received`, `Reviewed`, `Activated`) and the future Production separation. Improve density, clarity, accessibility, and responsiveness without adding workflow states or placeholder Production features. Treat Board columns as PM organization and Table filters as views over the same records, not as a workflow engine or Airtable status model. Internal Planning board code should use Planning/Merchandise language such as `planningCard`, `currentQueue`, `queues`, `deliverableRoute`, and `requiredToShoot`, not legacy `workOrder`, `currentGate`, `workstream`, workflow assignment, or public Readiness naming.
